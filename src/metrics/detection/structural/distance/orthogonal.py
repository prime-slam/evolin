...
def __calculate_averaged_orthogonal_distance(
    first_lines: np.ndarray, second_lines: np.ndarray, min_overlap=0.5
) -> np.ndarray:
    ...
    ) = __calculate_orthogonal_distance(first_lines, second_endpoints)

    (
        first_to_second_endpoint_distances,
        first_endpoints_projection_offsets,
    ) = __calculate_orthogonal_distance(second_lines, first_endpoints)
    ...
    second_endpoints_projection_offsets = second_endpoints_projection_offsets.reshape(
        (first_lines_number, second_lines_number, 2)
    )
    second_endpoints_overlaps = __calculate_endpoints_overlap(
        second_endpoints_projection_offsets
    )
    first_endpoints_projection_offsets = first_endpoints_projection_offsets.reshape(
        (second_lines_number, first_lines_number, 2)
    )
    first_endpoints_overlaps = __calculate_endpoints_overlap(
        first_endpoints_projection_offsets
    ).T
    overlaps = (second_endpoints_overlaps + first_endpoints_overlaps) / 2

    orthogonal_distances[overlaps < min_overlap] = np.inf

    return orthogonal_distances


def __calculate_orthogonal_distance(
    lines: np.ndarray, points: np.ndarray
...
def __calculate_endpoints_overlap(endpoints_offsets: np.ndarray) -> np.ndarray:
